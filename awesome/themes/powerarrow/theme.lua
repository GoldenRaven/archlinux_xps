...
lain.widget.contrib.task.attach(task, {
    ...
    show_cmd = "task | sed -r 's/\\x1B\\[([0-9]{1,2}(;[0-9]{1,2})?)?[mGK]//g';xsel -ob|head -n 20"
})
...
local mail = lain.widget.imap({
    timeout  = 180,
    server   = "pop.lzu.edu.cn",
    mail     = "ligy13@lzu.edu.cn",
    password = "sandbox_lilgy",
    settings = function()
        if mailcount > 0 then
...
local musicplr = awful.util.terminal .. " -title Music -g 130x34-320+16 -e mocp"
...
function theme.at_screen_connect(s)
    ...
    s.mywibox:setup {
        ...
        { -- Right widgets
            ...
            --]]
            arrow("#343434", theme.bg_normal),
            wibox.container.background(wibox.container.margin(wibox.widget { volicon, theme.volume.widget, layout = wibox.layout.align.horizontal }, 3, 6), theme.bg_focus),
            arrow(theme.bg_normal, "#343434"),
            ...
            s.mylayoutbox,
            arrow("alpha", "#4B3B51"),
            wibox.container.background(wibox.container.margin(wibox.widget { mytextclock, layout = wibox.layout.align.horizontal }, 3, 3), "#4B3B51"),
        },
   p }
end
